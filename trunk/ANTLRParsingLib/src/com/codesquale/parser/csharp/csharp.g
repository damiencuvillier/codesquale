header 
{
package com.codesquale.parser.csharp;
...
}
{
		...
		{
			System.out.println(ex + "\t[CSParser.reportError]");
			errorOccured = true;
		}
		
		protected String SetTypeName(antlr.CommonAST typenode)
		{
			if(typenode != null)
			{
				String val = typenode.getText();
				antlr.CommonAST child = (antlr.CommonAST)typenode.getFirstChild();
				while(child != null)
				{
					if(child.getType() ==  CSharpTokenTypes.ArrayRank)
					{
						val += "["+child.getText()+"]";
					}
					else if(child.getType() == CSharpTokenTypes.STAR)
					{
						val += "*";
					}
...
{int cnt=1;}  
	 : lb:LBRACK^ ( COMMA! {cnt++;} )* RBRACK!
	 		{#lb.setType(ArrayRank); #lb.setText(Integer.toString(cnt));}
   ;
...
for_statement!
	:	fr:FOR	lp:LPAREN	(init:for_initializer)?	se1:SEMI	(test:expression)?	se2:SEMI	(incex:expression_list)?	rp:RPAREN	st:embedded_statement 
...
foreach_statement! 
	:	frea:FOREACH   lp:LPAREN tp:type   id:identifier   i:IN   ex:expression	RPAREN   st:embedded_statement 
...
namespace_declaration 
	:	ns:NAMESPACE   tp:type_name   namespace_body    
...
using_directive
	:	us:USING^  tp:type_name (	ASSIGN! imp:type_name SEMI!
														...
														|	SEMI!
															{#us.setType(UsingNode); #us.setText("using");}
														)
	;
...
class_declaration[CommonAST attr, CommonAST mods] 
	:	cl:CLASS	id:identifier   (class_base)?   class_body	(SEMI!)? 
...
indexer_declaration_no_interface![CommonAST attr, CommonAST mods, CommonAST tp]
	:	THIS!	lb:LBRACK!	fpl:formal_parameter_list	RBRACK!	LBRACE!	adecl:accessor_declarations	RBRACE! 
...
indexer_declaration_interface![CommonAST attr, CommonAST mods, CommonAST tp]
	:	base:indexer_base	lb:LBRACK	fpl:formal_parameter_list	RBRACK	LBRACE	adecl:accessor_declarations	RBRACE
...
conversion_operator_declaration![CommonAST attr, CommonAST mods]
	:	conv_op:conversion_operator	op:OPERATOR	tp_out:type	LPAREN	tp_in:type	id:identifier	RPAREN	bd:body
...
struct_declaration[CommonAST attr, CommonAST mods]
	:	st:STRUCT   id:identifier   (base_interfaces)?   class_body   (SEMI!)? 
...
interface_declaration[CommonAST attr, CommonAST mods]
	:	inf:INTERFACE	id:identifier	(base_interfaces)?	interface_body   (SEMI!)?
...
enum_declaration[CommonAST attr, CommonAST mods] 
	:	en:ENUM   id:identifier   (enum_base)?   enum_body   (SEMI!)? 
...
delegate_declaration![CommonAST attr, CommonAST mods] 
	:	del:DELEGATE   tp:type   id:identifier   LPAREN   (params:formal_parameter_list)?   RPAREN   SEMI 
...
protected
NEW_LINE	// carriage return character followed by possible line feed character	
	:	(
	{ LA(2)=='\n' }? '\r' '\n'	
		|	'\r'
		|	'\n'
